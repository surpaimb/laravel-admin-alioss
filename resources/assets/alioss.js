...
(function () {
  ...
  function init_upload(id, csrf_token) {
    ...
    var uploader = new plupload.Uploader({
      ...
      url: "",
      flash_swf_url:
        "/vendor/laravel-admin-ext/alioss-upload/plupload/Moxie.swf",
      silverlight_xap_url:
        "/vendor/laravel-admin-ext/alioss-upload/plupload/Moxie.xap",
        ...
        BeforeUpload: function (up, file) {
          if (multi) {
            ...
            $("#" + id + "_container").append(
              '<div id="' +
                file.id +
                '" class="show_upload_pic_item"><div class="alioss_percent">0%</div></div>'
            );
          } else {
            // 单图
            browse_button
              ...
              .append(
                '<div id="' +
                  file.id +
                  '"><div class="alioss_percent">0%</div></div>'
              );
          }
          file_ext = get_suffix(file.name); //后缀名
          filename_new =
            Date.parse(new Date()) / 1000 + "_" + random_string(10) + file_ext;
  ...
  window.alioss_del_file = function (obj, type, edit) {
    ...
    if (type) {
      ...
      // 单图
      var show_upload_pic_item = $(obj)
        .attr("data-filename", "")
        .parents(".show_upload_pic_item");
      show_upload_pic_item
        .find("img.Js_alioss_btn")
        .attr("src", "/vendor/laravel-admin-ext/alioss-upload/pic_add.png");
